        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="space-y-2">
                @foreach(range(1, 12) as $month)
                    @php $count = $monthlyUsers[$month] ?? 0; @endphp
                    <div class="flex items-center">
                        <span class="w-20 text-sm">{{ date('F', mktime(0, 0, 0, $month, 1)) }}</span>
                        <div class="flex-1 bg-gray-200 rounded-full h-6 mr-4">
                            <div class="bg-blue-600 h-6 rounded-full" style="width: {{ min(100, ($count / max(1, max($monthlyUsers))) * 100) }}%"></div>
                        </div>
