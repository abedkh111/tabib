        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-gray-600">تعديل معلومات الكورس: {{ $course->title }}</p>
                </div>
            </div>
            <div class="flex items-center space-x-4 space-x-reverse">
                <a href="{{ route('admin.courses.show', $course) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="short_description" name="short_description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">{{ old('short_description', $course->short_description) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">{{ old('description', $course->description) }}</textarea>
                </div>
                        ...
                        <select id="specialization_id" name="specialization_id" required
                            ...
                            @foreach($specializations as $spec)
                                <option value="{{ $spec->id }}" 
                                        {{ ($course->specialization_id == $spec->id || old('specialization_id') == $spec->id) ? 'selected' : '' }}>
                                    {{ $spec->name_ar }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="instructor_id" name="instructor_id" required
                            ...
                            @foreach($instructors as $instructor)
                                <option value="{{ $instructor->id }}" 
                                        {{ ($course->instructor_id == $instructor->id || old('instructor_id') == $instructor->id) ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="discounted_price" name="discounted_price" 
                               value="{{ old('discounted_price', $course->discounted_price) }}" step="0.01" min="0"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="level" name="level"
                            ...
                            <option value="beginner" {{ ($course->level == 'beginner' || old('level') == 'beginner') ? 'selected' : '' }}>مبتدئ</option>
                            <option value="intermediate" {{ ($course->level == 'intermediate' || old('level') == 'intermediate') ? 'selected' : '' }}>متوسط</option>
                            <option value="advanced" {{ ($course->level == 'advanced' || old('level') == 'advanced') ? 'selected' : '' }}>متقدم</option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="flex items-center space-x-6 space-x-reverse">
                    <div class="flex items-center">
                        <input type="checkbox" id="is_published" name="is_published" value="1" 
                               {{ ($course->is_published || old('is_published')) ? 'checked' : '' }}
